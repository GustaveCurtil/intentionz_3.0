...
@section('main')
    ...
    <form action="create-event" method="post" enctype="multipart/form-data">
        ...
        <fieldset>
            ...
            <input type="date" name="datum" id="datum" required @if ($errors->any()) value="{{ old('datum') }}" @endif><input type="time" name="tijd" id="tijd" required @if ($errors->any()) value="{{ old('tijd') }}" @endif>          
        </fieldset>
        <fieldset>
            <legend>poster</legend>
            <label for="foto">kies een foto:</label>
        ...
        <fieldset>
            ...
            <textarea name="beschrijving" id="beschrijving" maxlength="2000" placeholder="beschrijving">@if ($errors->any()) {{ old('beschrijving') }} @endif</textarea>
